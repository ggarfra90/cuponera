var groupviewfrozen = $.extend({}, $.fn.datagrid.defaults.view, {
	render: function(target, container, frozen){
		...
		for(var i=0; i<groups.length; i++){
                    var group = groups[i];
                    table.push('<div class="datagrid-group" group-index=' + i + ' style="width:100%;height:25px;overflow:hidden;border-bottom:1px solid #ccc;">');
                    table.push('<table cellspacing="0" cellpadding="0" border="0" style="height:100%"><tbody>');
                    table.push('<tr>');
                    table.push('<td style="border:0;">');
                    var tbId = targetId + '_nfz' + i;
                    if (frozen){
                        table.push('<span style="color:#666;font-weight:bold;">');
                        table.push(opts.groupFormatter.call(target, group.gvalue, group.rows));
                        table.push('</span>');
                        tbId = targetId + '_fz' + i;
                    }
                    table.push('</td>');
                    ...
                    table.push('<table id="' + tbId + '" class="datagrid-btable" cellspacing="0" cellpadding="0" border="0"><tbody>');
		...
		$(container).html(table.join(''));
                // Buscamos los elementos no frozen
//                for(var i=0; i<groups.length; i++){
//                    // obtenemos el ancho del elemnto nofrozen
//                    var wd = $('#nfz' + i).width();
//                    // obtenenmos el elemento anterior al table, en este caso nuestro div
//                    var div = $('#nfz' + i).prev();
//                    // seteamos el nuevo ancho.
//                    div.css('width', wd);
//                }
	},
	...
	onAfterRender: function(target){
		...
		$('<td style="border:0"><div class="datagrid-row-expander datagrid-row-collapse" style="width:19px;height:16px;cursor:pointer"></div></td>').insertBefore(group.find('td'));
	...
	onBeforeRender: function(target, rows){
		...
		for(var i=0; i<rows.length; i++){
                    ...
                    if (!group){
                        ...
                        var columnTitle = '';
                        if(!isEmpty(cols) && cols.length > 0 && !isEmpty(cols[0])) {
                            cols = cols[0];
                            for (var j=0; j<cols.length; j++) {
                                if(cols[j].field === opts.groupField) {
                                    columnTitle = cols[j].title; break;
                                }
                            }
                        }
                        var lblEmptyGroup = isEmpty(opts.lblEmptyGroup) ? 'Vacío' : opts.lblEmptyGroup;
                        var value = isEmpty(row[opts.groupField]) ? '(' + lblEmptyGroup + ')' : row[opts.groupField];
                        value = isEmpty(columnTitle) ? value : columnTitle + ': ' + value;
                        group = {
                                gvalue: value,
		...
		function getGroup(fvalue){
                    for(var i=0; i<groups.length; i++){
                        var group = groups[i];
                        if (String(group.fvalue) === String(fvalue)) {
                                return group;
                        }
                    }
                    return null;
