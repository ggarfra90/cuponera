...
function isEmpty(value)
{
    ...
    if ($.type(value) === 'string' && value.length <= 0) return true;
    if ($.type(value) === 'array' && value.length === 0) return true;
    if ($.type(value) === 'number' && isNaN(parseInt(value))) return true;
...
function isEmptyData(data)
{
    ...
    {
        if (!hasPropiertyObject(data, 'rows')) return true;
...
{
    if ($.type(value) === JSType.UNDEFINED) return false;
    if (value === null) return false;
    
    if ($.type(value) === JSType.BOOLEAN) return true;
...
function crearWindow(title,divMensaje,messagerDefault){
    ...
    win.window({title:title,noheader:(title?false:true),width:300,height:"auto",modal:true,collapsible:false,minimizable:false,maximizable:false,resizable:false,onClose:function(){
    setTimeout(function(){
    win.window("destroy");
    },100);
    }});
...
{
    if (!hasPropiertyObject(obj, propierty)) return null;
    ...
    $('#buscadorButtonBuscarToolbar').tooltip({  
        ...
        onShow: function(){
            ...
            buttonSearch.onclick =  function(){
                if(!$('#buscadorFormCriterios').form('validate')) return;
                componentes.onClickBuscar(t);
            };
        },
        onPosition: function(left, top) {
            var _1b3 = $.data(this, "tooltip");
            if (!_1b3 || !_1b3.tip) return;
...
componentes.getTagMensaje= function (campo, valor, strMensaje){
    if (isEmpty(trim(valor))) return strMensaje;
    if (strMensaje !== "") strMensaje += " | ";
...
acciones.iniciaAjaxTest = function(componenteId, functionSuccess){
    ax.addParam(PARAM_COMPONENTE_ID, componenteId);
    ax.setSuccess(functionSuccess);
//    ax.addEventListener("onSuccess", functionSuccess);
    ax.addEventListener("onError", acciones.onErrorAjaxp);
}
...
editor.getMultiSelectText = function (dgNombre, index, fieldGrid, fieldMulti){
    if (editor.isEmptyMultiSelect(dgNombre, index, fieldGrid)) return '';
...
datex.formatoImaginaDG = function(valor){
    if (valor == '0000-00-00 00:00:00') {
        ...
        if (isEmpty(d) || d > 31 || d < 1) return "";
        var m = parseInt(ss[1],10);
        if (isEmpty(m) || m > 12 || m < 1) return "";
        var y = parseInt(ss[0],10);
        if (isEmpty(y) || y > 9999 || y < 1000) return "";
...
datex.parserControlador = function(s){ 
    ...
    if (!datex.isDateValida(oFecha)) return "";
...
datex.getValor = function (componente){
    ...
    if (valor == "") $("#"+componente).datebox('setValue', "");
    return valor;
}
...
function dia_semana(str){   
    var fecha="";
    var anio=0;
    var mesi=0;
    var dias=0;
    anio =str.substring(0,4);
        mesi =parseInt(str.substring(5,7));
        dias=parseInt(str.slice(8-10));
       fecha=dias+"/"+mesi+"/"+anio;
    fecha=fecha.split('/');  
    if(fecha.length!=3){  
            return null;  
    }  
    ...
    var semana=['Dom', 'Lun', 'Mar', 'Mie', 'Jue', 'Vie', 'Sáb'];  
        var meses = new Array ("NN","Ene","Feb","Mar","Abr","May","Jun","Jul","Ago","Sep","Oct","Nov","Dic");
    ...
    return semana[Math.ceil(Math.ceil(Math.ceil((anno-1)%7)+Math.ceil((Math.floor((anno-1)/4)-Math.floor((3*(Math.floor((anno-1)/100)+1))/4))%7)+mes+dia%7)%7)]+", "+dias+" de "+meses[mesi]+" del "+anio;  
}