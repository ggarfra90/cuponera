...
require_once __DIR__ . '/../../modelo/imatecsys/Componente.php';

require_once __DIR__ . '/../../modelo/cuponera/Usuario.php';

$_execute_controller = ControladorEnrutador::getInstance();
...
// <editor-fold defaultstate="collapsed" desc="funciones principales">
    ...
    public static function getInstance() {
        if (self::$instance === NULL) {
            self::$instance = new ControladorEnrutador();
        }
    ...
    public function __construct() {
        ...
        switch ($this->requestType){
            ...
            case self::FORMAT_JSON_EASYUI:
                // en ambos casos se obtiene defrente el response
                $this->getResponse();
                break;
            case self::FORMAT_OBJECT:
            default :
                // en estos casos se limpian los parametros xciacaso y se espera a 
                // que se llame a la función getResponse();
                $this->param->setParametros(NULL);
                break;
        } 
    ...
    public function getResponse() {
        try {
            ...
            $this->verificaSesion();
            $this->getComponente(); // Seteamos las variables de componente
            ...
            $controlador->params = $this->param->getParametros();
            $response = $controlador->$accion();
            $controlador->setCommitTransaction();
            $this->mensaje_emergente = $controlador->getMensajeEmergente();
            $controlador->clearConnection();
    ...
    private function setResponse($response){
        ...
        switch ($this->requestType) {
            case self::FORMAT_JSON:
                ...
                break;
            case self::FORMAT_JSON_EASYUI:
                $this->formatJSONEasyUI($response);
                break;
        }
    }
...
// <editor-fold defaultstate="collapsed" desc="functiones-de-salida">
     ...
     * imprime el objeto obtenido
     */
    private function formatJSONEasyUI($response) {
        $newresponse = array();
        if (!$this->error->has_error) {
            $newresponse['status'] = "ok";
            $newresponse[Configuraciones::PARAM_ACCION_NAME] = $this->param->accion;
            ...
            else
            {
                ...
                $newresponse['total'] = count($response);
            }
            ...
            $newresponse['key'] = 0;
            $newresponse['type'] = $this->error->getErrorTipo();
            $newresponse['rows'] = Array();
            $newresponse['total'] = 0;
            $newresponse['catch'] = $this->errors_catch;
            
        }
        if (!ObjectUtil::isEmpty($this->mensaje_emergente))
                $newresponse[self::RESPONSE_MENSAJE_EMERGENTE] = $this->mensaje_emergente;
...
//        $idUsuBD = $UsuBD[0]["id"];
//        if ($idUsuBD <= 0) $this->verificaSessionError();
...
//        $this->param->addParametro(Configuraciones::PARAM_COD_AD, $cod_ad);   
        ...
        $UsuBD = Usuario::create()->getUsuarioID($cod_ad);
        $idUsuBD = $UsuBD[0]["usu_id"];
        if ($idUsuBD <= 0) $this->verificaSessionError();
        $this->param->addParametro("id", $idUsuBD);
        $this->param->addParametro(Configuraciones::PARAM_COD_AD, $cod_ad);    
    }
...
//        Util::borrarCookie();
    ...
    private function getComponente() {
        //Obtenemos el componente en base al codigo solicitado
        $v = Componente::create()->getComponenteById($this->param->componenteId);
        
        if (ObjectUtil::isEmpty($v) || count($v) <= 0)
            throw new \WarningException("No tenemos inventariado el componente especificado");  
           
        $this->url_base = $v[0]["controlador_url"];
        $this->nombre_archivo = $v[0]["controlador_nombre"];
