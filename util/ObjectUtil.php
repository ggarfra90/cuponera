...
class ObjectUtil {
    ...
    static public function isEmpty($object)
    {
        ...
        if (is_string($object) && strlen($object) <= 0) return true;
        if (is_array($object) && empty($object)) return true;
        if (is_numeric($object) && is_nan($object)) return true;
    ...
    {
        if (!self::hasPropiertyValue($object, $propierty)) return null;
    ...
    {
        if (self::isEmpty($propierty)) return false;
        if (self::isEmpty($object)) return false;
    ...
    static public function formatDateTime($datetime){
        if ($datetime!= null) return date_format(new DateTime($datetime), 'Y-m-d H:i');
        return null;
    }
    ...
    static public function getArrayDeCampo($array, $campo) {
        if (self::isEmpty($array)) return null;
        $respuesta = array();
        $objeto = $array[0];
        if (is_array($objeto)){
